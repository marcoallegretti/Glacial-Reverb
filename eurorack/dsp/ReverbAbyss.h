...
class ReverbAbyss : public ReverbModel
{
public:
   explicit       ReverbAbyss (float sample_freq);
...
private:
   ...
   float          grain (const std::array<float,N> & b, float phase) const
   {
      ...
      return read (b, phase * _len) * w1 + read (b, p2 * _len) * w2;
   }
...
inline ReverbAbyss::ReverbAbyss (float sample_freq)
:  _sr (sample_freq)
,  _tank (sample_freq)
,  _buf (erb::make_sdram<Buffers> ())
{
   ...
   _inc = 0.5f / _len;              // ratio 0.5 = octave down
