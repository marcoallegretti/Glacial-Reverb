...
KoloredVerbDsp::KoloredVerbDsp (float sample_freq)
...
KoloredVerbDsp::~KoloredVerbDsp ()
{
   delete [] _pre_delay_buf_l;
   delete [] _pre_delay_buf_r;
}

void  KoloredVerbDsp::set_mix (float mix)
{
   if (mix < 0.f) mix = 0.f;
   if (mix > 1.f) mix = 1.f;
   _mix = mix;
...
void  KoloredVerbDsp::set_decay (float decay)
{
...
void  KoloredVerbDsp::set_pre_delay (float pre_delay)
{
...
void  KoloredVerbDsp::set_frozen_level (float level)
{
...
void  KoloredVerbDsp::set_frozen_mix (float mix)
{
   if (mix < 0.f) mix = 0.f;
   if (mix > 1.f) mix = 1.f;
   _frozen_mix = mix;
...
void  KoloredVerbDsp::set_freeze (bool freeze)
{
   if (freeze != _freeze)
   {
      _freeze_hold_s = 0.f;
...
void  KoloredVerbDsp::set_fx (float fx)
{
   if (fx < 0.f) fx = 0.f;
   if (fx > 1.f) fx = 1.f;
   _fx_param = fx;
...
void  KoloredVerbDsp::set_tank (bool enabled)
{
   _tank_enabled = enabled;
}

dsp::ReverbModel *   KoloredVerbDsp::model_for_id (int id)
{
   switch (id)
   {
...
void  KoloredVerbDsp::set_fx_type (int fx_index)
{
...
dsp::TailFx *   KoloredVerbDsp::tail_for_id (int id)
{
   switch (id)
   {
...
void  KoloredVerbDsp::set_tail_fx (int tail_index)
{
...
void  KoloredVerbDsp::set_tail_amount (float amount)
{
...
void  KoloredVerbDsp::set_rate (float rate)  { (void) rate; }
void  KoloredVerbDsp::set_depth (float depth) { (void) depth; }

void  KoloredVerbDsp::set_duck (float duck)
{
...
void  KoloredVerbDsp::process (float * const out [], const float * const in [], std::size_t size)
{
   auto in_left  = in [0];
   auto in_right = in [1];
   auto out_left  = out [0];
